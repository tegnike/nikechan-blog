...
- サムネイル標準サイズ: 1200x630px、`thumbnail.png` 固定推奨。OGP向けの標準サイズなので、これより大きい画像をコミットしない。生成元が大きい場合は公開前に1200x630へリサイズする
- 画像生成プロンプト: `public/static/images/posts/{slug}/image-prompts.md`
...
6. **slug 候補**
7. **画像方針**（サムネイルの主役・入れたい文字・挿絵や図解が必要な章）
...
- 画像ファイル名は `thumbnail.png`, `01.png`, `02.png` のように固定し、本文の登場順と `image-prompts.md` の順番を合わせる
- キャプション行は画像直後に短く置く。説明として自然なものにし、唐突なツッコミだけのキャプションは避ける
- 補足・注意書き・出典への敬意など、本文から少し目立たせたい短いメッセージは黄色の `:::message` 記法を使える
...
- **サムネイル**: 記事タイトルや主張が一目で分かる構図にする。1200x630px、`thumbnail.png`、ブログ一覧とOGPで縮小されても読める文字量に抑える。ブログ内では拡大表示しないため、1200x630を上限/標準とし、生成画像が大きい場合は `magick input.png -resize '1200x630^' -gravity center -extent 1200x630 -strip thumbnail.png` のようにリサイズしてから配置する
...
- **整合性**: frontmatter の `thumbnail`、本文の画像パス、`image-prompts.md` の見出しが同じファイル名になっていることを確認する
...
- [ ] 唐突なツッコミ / 独り言が混ざっていない。感情表現を入れる場合も、本文の文脈に自然につながっている
...
- [ ] `public/static/images/posts/{slug}/image-prompts.md` を作成し、`thumbnail.png` と本文中の画像すべてのプロンプトが揃っている
- [ ] サムネイルのプロンプトにサイズ（1200×630px）、レイアウト、文字情報、制約が入っている
- [ ] `thumbnail.png` の実ファイルが1200×630pxに収まっている（生成元が大きい場合はリサイズ済み）
- [ ] 挿絵・図解プロンプトが本文の章と対応しており、単なる装飾ではなく理解を助ける内容になっている
...
- **黄色メッセージ枠は `:::message` で囲む**。補足・注意書き・出典注記などに使う。赤い警告枠は未実装なので使わない
