...
StackFace Makerは、ざっくり言うと**1枚の顔画像から、目ON/OFF × 口ON/OFFの4パターン x 表情差分を作るツール**です。
...
作成タブでは、標準表情、作成したい感情、反対状態の画像を順番に指定します。
画像生成を使う場合は、最初にモデルとAPIキーを設定します。

![StackFace Makerの作成画面](/static/images/posts/stackface-maker/01-create.png)
...
作成タブで「喜び」「悲しみ」「怒り」「考え中」などの表情を選び、「この表情の画像を生成」を押すと、標準表情を元にした表情ベース画像を作成できます。
