...
description: "AIにゲームを遊ばせるとき、画面認識以外にどんな状態取得や操作のルートがあるのかを整理しました。Pokémon Showdown、Slay the Spire、Minecraftなどを例に、AIが遊びやすいゲームと難しいゲームの違いを考えます。"
...
その中で、AIにプレイまでさせるなら「テキストや構造化データで状態が取れて、操作も返しやすいゲームを選ぶとよい」という話をしました。
上記の記事では Pokémon Showdown を例に、ゲーム画面をマルチモーダルAIで解析するのではなく、ゲーム状態をテキストとして取ってLLMに渡し、選んだ行動をゲームに返す構成を紹介しています。
...
さらに、選んだ技や交代先をBot側からバトルサーバーへ送れるので、状態を見るだけでなく実際に対戦を進められます。
...
LLMにそのまま強く打たせるのは別問題ですが、少なくとも「状態を渡して行動を返し、その手を盤面に反映する」パイプラインは作りやすいです。
...
ただし、[CommunicationMod](https://github.com/ForgottenArbiter/CommunicationMod) というModを入れると、外部プロセスと `stdin` / `stdout` で双方向に通信できます。
ゲーム状態のJSONを受け取るだけでなく、外部プロセスからコマンドを返して、カードを使う、ターンを終了する、報酬やイベントの選択肢を選ぶ、といった操作も実行できます。
...
例えば `PLAY 1 0` なら手札の1枚目を0番目の敵に使い、`END` ならターン終了、`CHOOSE 0` なら提示された選択肢の0番目を選ぶ、というようにゲーム側へ操作を送れます。
...
外部エージェントが `open door` や `take key` のようなテキストコマンドを返すと、それをゲームに入力して次の観測を受け取る形になります。

NetHack も、[NetHack Learning Environment](https://github.com/facebookresearch/nle) や MiniHack のような研究環境があります。
こちらも観測を受け取って、移動や攻撃などのアクションをコマンドとして返せます。
ゲーム自体は難しいですが、少なくとも「画面画像をLLMに見せて、キーボードを無理やり押す」以外のルートが用意されています。
...
ModやLua APIを使えば、状態取得だけでなく、クラフト、配置、移動などの操作もプログラム側から実行できます。
...
でも実際には、DOM、通信、プロトコル、Mod、Botライブラリ、エミュレータ、研究環境など、状態を取ったり、操作を送ったりするルートはかなりあります。
...
特に Slay the Spire は状態をJSONで受け取り、操作もコマンドで返せるので、LLMプレイヤーを作る題材として普通に良さそうです。
