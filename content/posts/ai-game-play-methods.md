...
操作も実況セリフもLLMの出力をそのまま解釈して処理するので、遅延も誤認識も発生する余地がありません。
...
弱点は、**対応するゲームが限られる**ことです。
テキストで状態が取得できるゲームでないと成立しないので、「任意のゲームを遊ばせたい」用途には向きません。
...
- **AIにプレイも実況もやらせたい** → 案3一択。対応ゲームは限られる分、できたときの完成度は高い
